...
struct mdp4_overlay_ctrl {
	...
	uint32_t mixer_cfg[MDP4_MIXER_MAX];
	uint32_t flush[MDP4_MIXER_MAX];
	struct iommu_free_list iommu_free[MDP4_MIXER_MAX];
	struct iommu_free_list iommu_free_prev[MDP4_MIXER_MAX];
	uint32_t dmap_cfg[5];
	uint32_t cs_controller;
	uint32_t panel_3d;
	uint32_t panel_mode;
	uint32_t mixer0_played;
	uint32_t mixer1_played;
	uint32_t mixer2_played;
} mdp4_overlay_db = {
...
static void mdp4_overlay_iommu_2freelist(int mixer, struct ion_handle *ihdl)
{
...
static int mdp4_overlay_iommu_map_buf(int mem_id,
	struct mdp4_overlay_pipe *pipe, unsigned int plane,
	dma_addr_t *start, size_t *len,
	struct ion_handle **srcp_ihdl)
{
	...
	*srcp_ihdl = ion_import_dma_buf(display_iclient, mem_id);
	...
	ion_phys(display_iclient, *srcp_ihdl, start, len);
	...
	pr_debug("%s: ndx=%d plane=%d prev=0x%p cur=0x%p start=%pad len=%zu\n",
...
static void fill_black_screen(bool on, uint8_t pipe_num, uint8_t mixer_num)
{
	uint32_t reg_base        = 0x010000;
	uint32_t const_color_reg = reg_base * (pipe_num + 2) + 0x1008;
	uint32_t src_fmt_reg     = reg_base * (pipe_num + 2) + 0x50;
	uint32_t color           = 0x00000000;
	uint32_t temp_src_format = 0x00000000;
	uint8_t  bit             = pipe_num + 2;
...
static void mdp4_overlay_vg_get_src_offset(struct mdp4_overlay_pipe *pipe,
	char *vg_base, uint32_t *luma_off, uint32_t *chroma_off)
{
	uint32_t src_xy;
	*luma_off = 0;
	*chroma_off = 0;
...
static void mdp4_overlay_vg_setup(struct mdp4_overlay_pipe *pipe)
{
	char *vg_base;
...
static void transp_color_key(int format, uint32_t transp,
...
static int get_img(struct msmfb_data *img, struct fb_info *info,
	struct mdp4_overlay_pipe *pipe, unsigned int plane,
	dma_addr_t *start, size_t *len, struct file **srcp_file,
...
int mdp4_overlay_unset(struct fb_info *info, int ndx)
{
	...
	if (pipe->mixer_num != MDP4_MIXER0) {	/* mixer1, DTV, ATV */
...
static void tile_samsung(struct tile_desc *tp)
{
	/*
	 * each row of samsung tile consists of two tiles in height
	 * and two tiles in width which means width should align to
	 * 64 x 2 uint8_ts and height should align to 32 x 2 uint8_ts.
	 * video decoder generate two tiles in width and one tile
	 * in height which ends up height align to 32 X 1 uint8_ts.
	 */
	tp->width = 64;		/* 64 uint8_ts */
	tp->row_tile_w = 2;	/* 2 tiles per row's width */
	tp->height = 32;	/* 32 uint8_ts */
	tp->row_tile_h = 1;	/* 1 tiles per row's height */
}

static uint32_t tile_mem_size(struct mdp4_overlay_pipe *pipe,
			      struct tile_desc *tp)
{
...
int mdp4_overlay_play(struct fb_info *info, struct msmfb_overlay_data *req)
{
	...
	img = &req->data;
	get_img(img, info, pipe, 0, &start, &len, NULL, NULL, &srcp0_ihdl);
	if (len == 0) {
		pr_err("%s: pmem Error\n", __func__);
		ret = -1;
		goto end;
	...
	pr_debug("%s: mixer=%d ndx=%x addr=%pad flags=%x pid=%d\n", __func__,
		pipe->mixer_num, pipe->pipe_ndx, &addr, pipe->flags,
							current->pid);
	...
	if (pipe->mixer_num == MDP4_MIXER0) {
		if (ctrl->panel_mode & MDP4_PANEL_MDDI)
			mdp4_mddi_pipe_queue(0, pipe); /* cndx = 0 */
	} else if (pipe->mixer_num == MDP4_MIXER1) {
		if (ctrl->panel_mode & MDP4_PANEL_DTV)
