...
static struct mdp_csc_cfg_data csc_cfg_matrix[CSC_MAX_BLOCKS] = {
	{
...
void mdp4_display_intf_sel(int output, ulong intf)
{
	...
	MSM_FB_DEBUG("%s: 0x%x\n", __func__, inpdw(MDP_BASE + 0x0038));
}
...
static void mdp4_fetch_cfg(uint32_t core_clk)
{
	uint32_t dmap_data, vg_data;
	char *base;
	int i;
...
static void mdp4_clear_lcdc(void)
{
	uint32_t bits;
...
static uint32_t vg_qseed_table0[] = {
	0x5556aaff, 0x00000000, 0x00000000, 0x00000000
};

static uint32_t vg_qseed_table1[] = {
	0x00000000, 0x20000000,
};

static uint32_t vg_qseed_table2[] = {
...
void mdp4_vg_qseed_init(int vp_num)
{
	...
	off = (uint32_t *)(MDP_BASE + MDP4_VIDEO_BASE + voff +
						MDP4_QSEED_TABLE0_OFF);
	for (i = 0; i < (sizeof(vg_qseed_table0) / sizeof(uint32_t)); i++) {
		...
		if (!((uint32_t)off & 0x3FF))
			wmb();
	}

	off = (uint32_t *)(MDP_BASE + MDP4_VIDEO_BASE + voff +
						MDP4_QSEED_TABLE1_OFF);
	for (i = 0; i < (sizeof(vg_qseed_table1) / sizeof(uint32_t)); i++) {
	...
	for (i = 0; i < (sizeof(vg_qseed_table2) / sizeof(uint32_t)); i++) {
...
static const char gc_lut[] = { /* linear */
...
static const uint32_t igc_video_lut[] = {	/* linear */
...
static const uint32_t igc_rgb_lut[] = {	/* linear */
	0x0, 0x10, 0x20, 0x30, 0x40, 0x50, 0x60, 0x70,
	0x80, 0x91, 0xA1, 0xB1, 0xC1, 0xD1, 0xE1, 0xF1,
	0x101, 0x111, 0x121, 0x131, 0x141, 0x151, 0x161, 0x171,
	0x181, 0x191, 0x1A2, 0x1B2, 0x1C2, 0x1D2, 0x1E2, 0x1F2,
...
int mdp4_allocate_writeback_buf(struct msm_fb_data_type *mfd, u32 mix_num)
{
	...
	buf->ihdl = ion_alloc(mfd->iclient, mfd->fb_size, SZ_4K,
			      mfd->mem_hid, 0);

	if (IS_ERR(buf->ihdl)) {
		ret = PTR_ERR(buf->ihdl);
		pr_err("%s: failed to allocate ret=%d\n",
		       __func__, ret);
		return ret;
	}

	ret = ion_phys(mfd->iclient, buf->ihdl, &addr, &len);
	if (ret) {
		pr_err("%s: failed to get physical address ret=%d\n",
		       __func__, ret);
		return ret;
	}
